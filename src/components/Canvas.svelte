<script>
	...
	import { 
		timer, cash, mana, collector_pos, bounce,
		basic_orb, light_orb, homing_orb, auto_fight, afford_fight,
	...
	$: { 
		calc_cps = 
			...
			((orbs.sub_spore_allocated + orbs.sub_spore_over)*$spore_orb.sub_value)) * (($bounce.power-30)/8) * ($bounce.auto_unlocked ? 1 : 0) +
	...
	const key_up = (e)=>{
		...
		else if (k == "d") console.log(orbs.homing.length);
...
<main bind:this={main} style="opacity: {$toggled ? "1" : "0"}; pointer-events: {$toggled ? "all" : "none"};">
	...
	<h3 id="cash">
		...
		Calculated Orbs: {format_num(calc_orbs)} -->
	</h3>  
